---
title: Get Started with Horizon UI - React & Tailwind CSS Components Library
description: Learn how to use Horizon UI components from this documentation to easily create elegant and flexible pages using Tailwind CSS and React.
navigation:
  [
    "quick-start",
    "installation",
    "npm",
    "yarn",
    "pnpm",
    "tailwindcss-config",
    "theme-provider",
    "example",
    "typescript",
  ]
github: quick-start
next: license
---

# Quick Start

<span id='quick-start' className='scroll-mt-48' />
Horizon UI is free and open-source components library for ReactJS and Tailwind CSS
inspired by Material Design.

<br />
## Installation
<span id='installation' className='scroll-mt-48' />

Learn how to use @material-tailwind/react components from this documentation to quickly and easily create elegant and flexible pages using Tailwind CSS.

@material-tailwind/react is working with Tailwind CSS classes and you need to have Tailwind CSS installed on your project - <a href="https://tailwindcss.com/docs/installation/framework-guides?ref=horizon-ui.com" target="_blank" className="font-medium hover:text-blue-500 transition-colors">Tailwind CSS Installation.</a>

<br />
## Using NPM

Install Horizon UI as a dependency using NPM by running the following command:

<span id='npm' className='scroll-mt-48' />

```bash
npm i @material-tailwind/react
```

---

## Using Yarn

Or use Yarn:

<span id='yarn' className='scroll-mt-48' />

```bash
yarn add @material-tailwind/react
```

---

## Using PNPM

<span id='pnpm' className='scroll-mt-48' />

```bash
pnpm i @material-tailwind/react
```

---

## TailwindCSS Configurations

<span id='tailwindcss-config' className='scroll-mt-48' />

Once you install @material-tailwind/react you need to wrap your tailwind css configurations with the <Code>withMT()</Code> function coming from @material-tailwind/react.

```js {1, 3, 9}
const withMT = require("@material-tailwind/react/utils/withMT");

module.exports = withMT({
  content: ["./src/**/*.{js,jsx,ts,tsx}"],
  theme: {
    extend: {},
  },
  plugins: [],
});
```

---

## Theme Provider

<span id='theme-provider' className='scroll-mt-48' />

@material-tailwind/react is comes with a theme provider that you can use to provide your own theme to your components. You need to wrap your entire application with the <Code>ThemeProvider</Code> coming from @material-tailwind/react.

```jsx {4, 7, 9}
import React from "react";
import ReactDOM from "react-dom";
import App from "App";
import { ThemeProvider } from "@material-tailwind/react";

ReactDOM.render(
  <ThemeProvider>
    <App />
  </ThemeProvider>,
  document.getElementById("root")
);
```

---

## Example

Now you're good to go and use @material-tailwind/react in your project.

<ComponentCard id='example'>
  <Button>Button</Button>
</ComponentCard>

```jsx showLineNumbers {1, 4}
import { Button } from "@material-tailwind/react";

export default function Example() {
  return <Button>Button</Button>;
}
```

---

## TypeScript

<span id='typescript' className='scroll-mt-48' />

@material-tailwind/react is a TypeScript based components library and you can easily use it in your TypeScript based projects.
